    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-transparent">
      ...
      <nav className="hidden md:flex w-24 lg:w-72 flex-col border-r border-[var(--border)] bg-[var(--nav-bg)] backdrop-blur-xl h-screen sticky top-0 p-6 overflow-hidden">
        ...
        <div className="space-y-4 flex-1">
          ...
          <NavItem isActive={activeView === 'create'} onClick={() => setActiveView('create')} label="Manifest" icon={<svg className="w-6 h-6 nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>} />
        </div>
        ...
        <div className="mt-auto space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 pt-4 border-t border-[var(--border)] border-dashed">
            <button 
              onClick={onLogout}
              title="Switch Identity"
              className="p-3 flex items-center justify-center rounded-xl bg-[var(--input-bg)] hover:bg-[var(--primary)] hover:text-white transition-all group"
            >
              <svg className="w-5 h-5 opacity-40 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
            </button>
            <button 
              onClick={onLogout}
              title="Sever Identity"
              className="p-3 flex items-center justify-center rounded-xl bg-[var(--input-bg)] hover:bg-red-500 hover:text-white transition-all group"
            >
              <svg className="w-5 h-5 opacity-40 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
            </button>
          </div>
          <p className="text-[8px] font-bold uppercase opacity-30 text-center tracking-tighter pt-1">Session Management</p>
        </div>
      </nav>
      ...
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md bg-[var(--nav-bg)] backdrop-blur-2xl border-2 border-[var(--border)] rounded-[2.5rem] p-2 flex items-center justify-between shadow-2xl">
        ...
        <MobileNavItem isActive={activeView === 'discovery'} onClick={() => setActiveView('discovery')} icon={<svg className="w-6 h-6 nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>} />
